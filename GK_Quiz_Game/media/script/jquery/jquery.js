...
function scaleStage() {
    ...
    // Calculate individual scale ratios
    const scaleX = window.innerWidth / config['stageWidth'];
    const scaleY = window.innerHeight / config['stageHeight'];

    // Choose the smaller of the two to maintain aspect ratio
    const zoomFactor = Math.min(scaleX, scaleY);

    // Calculate new width and height based on zoomFactor
    const newWidth = config['stageWidth'] * zoomFactor;
    const newHeight = config['stageHeight'] * zoomFactor;

    // Center the stage horizontally and vertically
    const leftPos = (window.innerWidth - newWidth) / 2;
    const topPos = (window.innerHeight - newHeight) / 2;

    // Save position and zoom globally if needed
    window.stageLeft = leftPos;
    window.rootLeftElmPos = leftPos;
    window.zoomFactor = zoomFactor;

    // Apply transform and positioning styles
    const styleObj = {
        transform: `scale(${zoomFactor})`,
        WebkitTransform: `scale(${zoomFactor})`,
        MozTransform: `scale(${zoomFactor})`,
        msTransform: `scale(${zoomFactor})`,
        OTransform: `scale(${zoomFactor})`,
